...
from app.models.sensor import sensor_data_hourly_avg_view, sensor_data_daily_avg_view, \
...
class CRUDSensorData:
    ...
    def get_average_from_aggregate_view(
        self,
        db: Session,
        *,
        sensor_id: str,
        from_date: datetime,
        to_date: datetime,
        interval: str # Z.B. '1h', '1d', '1w', '1M', '1y'
    ) -> Optional[List[Dict[str, Any]]]: # Rückgabe ist Liste von Dicts oder None wenn Intervall nicht unterstützt
        """
        Ruft Durchschnittswerte aus der passenden kontinuierlichen Aggregation (View) basierend auf dem Intervall ab.
        Gibt None zurück, wenn kein passendes Aggregat für das Intervall existiert.
        """
        # Mappe das Intervall auf die entsprechende Core Table View und Zeitspaltenname
        aggregate_map = {
            '1h': (sensor_data_hourly_avg_view, 'hour'),
            '1hour': (sensor_data_hourly_avg_view, 'hour'),
            '1d': (sensor_data_daily_avg_view, 'day'),
            '1day': (sensor_data_daily_avg_view, 'day'),
            '1w': (sensor_data_weekly_avg_view, 'week'),
            '1week': (sensor_data_weekly_avg_view, 'week'),
            '1m': (sensor_data_monthly_avg_view, 'month'),
            '1month': (sensor_data_monthly_avg_view, 'month'),
            '1M': (sensor_data_monthly_avg_view, 'month'),
            '1y': (sensor_data_yearly_avg_view, 'year'),
            '1year': (sensor_data_yearly_avg_view, 'year'),
        }

        # Prüfe, ob das Intervall unterstützt wird und hole die View und den Spaltennamen
        interval_lower = interval.lower()
        if interval_lower not in aggregate_map:
            return None 

        view, time_column_name = aggregate_map[interval_lower]

        query = db.query(
            column(time_column_name).label('time_bucket'), 
            view.c.average_value.label('aggregated_value')
        ) \
        .filter(view.c.sensor_id == sensor_id)  \
        .filter(column(time_column_name) >= from_date) \
        .filter(column(time_column_name) < to_date)  \
        .order_by(column(time_column_name))        
